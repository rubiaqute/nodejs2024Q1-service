...
export const isValidCreateUserPayload = (createUserPayload: CreateUserDto) => {
  const isOnlyKnownKeys = Object.keys(createUserPayload).every(
    (key) => key === 'login' || key === 'password',
  );
  const isValidValues = Object.values(createUserPayload).every(
...
export const isValidUpdatePasswordPayload = (
  updatePasswordPayload: UpdatePasswordDto,
) => {
  const isOnlyKnownKeys = Object.keys(updatePasswordPayload).every(
    (key) => key === 'oldPassword' || key === 'newPassword',
  );
  const isValidValues = Object.values(updatePasswordPayload).every(
    (value) => typeof value === 'string' && value.trim().length > 0,
  );
...
export const isValidCreateTrackPayload = (
  createTrackPayload: CreateTrackDto,
) => {
...
export const isValidUpdateTrackPayload = (
  updateTrackPayload: UpdateTrackDto,
) => {
...
export const isValidCreateArtistPayload = (
  createArtistPayload: CreateArtistDto,
) => {
  const isOnlyKnownKeys = Object.keys(createArtistPayload).every(
    (key) => key === 'name' || key === 'grammy',
  );
  const isValidName =
    typeof createArtistPayload.name === 'string' &&
    createArtistPayload.name.trim();
  const isValidGrammy = typeof createArtistPayload.grammy === 'boolean';

  return isOnlyKnownKeys && isValidName && isValidGrammy;
};

export const isValidUpdateArtistPayload = (
  updateArtistPayload: UpdateArtistDto,
) => {
  const isOnlyKnownKeys = Object.keys(updateArtistPayload).every(
    (key) => key === 'name' || key === 'grammy',
  );
  const isValidName =
    updateArtistPayload.name === undefined ||
    (typeof updateArtistPayload.name === 'string' &&
      updateArtistPayload.name.trim());
  const isValidGrammy =
    updateArtistPayload.grammy === undefined ||
    typeof updateArtistPayload.grammy === 'boolean';

  return isOnlyKnownKeys && isValidName && isValidGrammy;
};

export const isValidCreateAlbumPayload = (
  createAlbumPayload: CreateAlbumDto,
) => {
...
export const isValidUpdateAlbumPayload = (
  updateAlbumPayload: UpdateAlbumDto,
) => {
  ...
  const isValidYear =
    year === undefined || (typeof year === 'number' && year >= 0);
  const isValidArtistId = !artistId || uuidValidate(artistId);
